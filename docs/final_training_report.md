...
## 六-B、為何成績普遍在 0.60–0.70 之間？

> 以下分析完全基於合法競賽條件下（使用官方訓練集、不讀取 test GT、不使用外部標註資料）所觀察到的結構性限制。

### 根本原因：評分任務與資料集語義之間的根本矛盾

本競賽的評分指標是 **Macro F1（單標籤）**，但資料集的真實語義是**多標籤**。

Schopf et al. (2022) 原始資料集中，每篇醫學摘要本來就被多個 MeSH 概念標記，例如一篇關於「糖尿病患者的心臟病風險」的摘要，在原始 MeSH 標記下同時屬於：
- Class 2（digestive system diseases — 糖尿病屬代謝/消化相關）
- Class 4（cardiovascular diseases）
- Class 5（general pathological conditions）

將這個多標籤資料集強制轉成「每行只有一個標籤」並用 Macro F1 評分，本質上是在問模型：**「從語義上等效的多個標籤中，猜出標記者當下選了哪一個？」**
...
#### 天花板一：Class 5 的 F1 無法突破 ~0.55

| | 數值 |
|---|---|
| Class 5 在 train 的真實比例 | 32.1% |
| 模型最終預測 class 5 的比例 | 21.7%（314/1444） |
| Class 5 的最終 F1 | **0.5321** |

**為什麼 class 5 永遠最差？**

Class 5（general pathological conditions）是「其他」類別的代名詞。一篇摘要被標為 class 5 的理由，往往是「它跟其他四類的相關性都差不多」，沒有明確的語言特徵可以學習。模型看到的文字特徵（心臟、腫瘤、神經等關鍵字）都指向 class 1–4，class 5 的正樣本缺乏獨特的語言訊號。

更嚴重的是：class 5 的 recall 不足（模型低估了它的真實比例），prior adjustment 可以部分補救（把 class 5 的 log prob 往上調），但補救上限受限於——如果模型的 logit 對 class 5 完全沒有鑑別力，再怎麼調 prior 都是讓其他本來應該是 class 1–4 的樣本錯誤地被推到 class 5。

**Macro F1 受 class 5 的 F1 拉低約 0.03–0.05**。

---

#### 天花板二：Train–Test 文本重疊 41%，但標籤不一定一致

Test set 中約 589 個 unique 文本（41%）也出現在 train set 中。這看起來像「免費分數」，實際上卻是陷阱：

- 同一篇文章在 train 和 test 的標籤**可能不同**（因為原本就是多標籤，強制單標籤化後每次取哪個標籤是任意的）
- 即使模型「背下來」了 train 中看過的文本，test 的標籤仍可能是那篇文章的另一個合法標籤
- 這讓 train–test 重疊帶來的增益遠低於預期

---

#### 天花板三：OOF 估計系統性偏高，真實泛化能力受限

| 指標 | 數值 |
|---|---|
| 最佳 OOF proxy F1 | 0.6724 |
| 實際 LB | 0.65197 |
| Gap | **−0.0204** |

這個 gap 是穩定的（跨所有提交幾乎固定在 ±0.002 以內），說明它不是隨機誤差，而是系統性偏差：
...
| 嘗試 | 模型 | LB |
|---|---|---|
| PubMedBERT base | 109M params | 0.641 |
| PubMedBERT-large | 340M params | 0.646 |
| DeBERTa-v3-large | 400M params | N/A（bf16 崩潰） |

PubMedBERT base → large 帶來 +0.005 LB，這個增益符合預期（更大的模型，更好的語義表示）。但增益迅速遞減——因為**瓶頸不在模型容量，而在標籤本身的模糊性**。

對於「這篇文章是 class 4 還是 class 5？」這個問題，即使是 GPT-4 這種規模的模型，在缺乏「標記者當下的意圖」的情況下，也無法可靠地猜對。模型能做到的最好結果，是學到整個訓練集的「統計偏好」——這個統計偏好的上限，就是 0.65 附近。
...
假設最理想情況：
- 模型完美記住所有 train 中見過文本的標籤 → 幫助覆蓋 41% 的 test（589/1444）
- 對剩下 59% 的 test（855 筆）只能靠語言特徵，class 5 F1 最多 ~0.60

即使這樣估算：

```
理論上限 Macro F1 ≈
  前 4 類（class1–4）平均 F1 ≈ 0.74
  class 5 最佳估計 F1 ≈ 0.60
  Macro F1 = (0.74×4 + 0.60) / 5 ≈ 0.712
```

但這個 0.71 的理論上限建立在「完美記憶 train 中 41% 重疊文本」的假設上，實際上重疊文本的標籤一致性不到 100%，且 class 5 在 test 中的分布估計也有不確定性。

**合法條件下，0.65–0.68 是現實的天花板區間。**

---

### 小結

| 限制因素 | 對 Macro F1 的壓制估計 |
|---|---|
| Class 5 語義模糊（無獨特語言特徵） | −0.03~−0.05 |
| OOF–LB 系統性 gap（多標籤結構） | −0.020（固定） |
| Train–Test 重疊標籤不一致 | −0.01~−0.02 |
| 模型容量未達（DeBERTa bf16 失敗） | −0.01~−0.02（潛在） |

**這四個因素疊加，使得任何遵守競賽規則的方法，在 Macro F1 上幾乎必然落在 0.60–0.70 的區間內。**  
突破 0.70 的唯一路徑，是改變評分方式（改為多標籤 F1 或允許多個正確答案），或使用外部 MeSH 標籤作為監督訊號——兩者均超出本次競賽的合法範圍。
