...
| 檔案 | 用途 |
|---|---|
| `src/make_pseudo_labels.py` | 根據 final_d 預測，產生 pseudo-labeled train CSV |
| `src/train_bert_pseudo.py` | 載入擴充 train（含 PL），訓練 5-fold |
| `src/tta_predict.py` | 對既有模型做 TTA |

明早把這些腳本寫好 push GitHub，Colab 直接拉下來跑。
